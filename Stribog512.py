...
def Ekm(k,m):
    ...
    for i in range(1,len(K)):
        K[i]=LPS(BcisSHA1(K[i-1]^C[i-1],512))
    M=m
    for i in range(0,len(K)-1):
        M=LPS(Xk(K[i],M))
    M=СхемаГорнера1(Xk(K[len(K)-1],M),2)
    return M #На входе десятичные числа на выходе десятичное число

def gN(h,m,N):
    g=Ekm(LPS(BcisSHA1(h^N,512)),m)^h^m
...
def Стрибог512(M):
    ...
    M1=M
    while(len(M1)>=512):
        M2,m=M1[0:len(M1)-512],СхемаГорнера1(M1[len(M1)-512:len(M1)],2)
        h=gN(h,m,N)
        N=(N+512)%(2**512)
        E=(E+m)%(2**512)
        M1=M2
    M2=[0]*(511-len(M1))+[1]+M1
    m=СхемаГорнера1(M2,2)
    h=gN(h,m,N)
    N=(N+len(M1))%(2**512)
    E=(E+m)%(2**512)
    h=gN(h,N,0)
    h=gN(h,E,0)
    H=BcisСтрибог(h,256,64)
    H.reverse()
    h=СхемаГорнера1(H,256)
    return h

def СхемаГорнера1(a=[0],x=0):
    i=0
    q=0
    while(i<len(a)):
        q=int(a[i])+x*q
